export const getPlatform = ReleasesGithubReducerData => {
    let buttonPlatformContent = 'CLI';
    let linkPlatform = 'https://github.com/containerum/chkit/releases/latest';
    let version = Object.keys(ReleasesGithubReducerData).length ? ReleasesGithubReducerData.tag_name : 'v.2.14';
    let size = '222202';
    ...
    if (typeof window !== 'undefined') {
        ...
        if (windowAgent) {
            ...
            Object.keys(ReleasesGithubReducerData).length ? ReleasesGithubReducerData.assets.map((item) => {
