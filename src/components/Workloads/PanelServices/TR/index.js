...
export default class TR extends Component {
  ...
  getNumPages() {
    var numPages = Math.floor(this.props.data.services.length / this.props.pageSize)
    ...
    return (
      <tr>
        <td className='width_td'></td>
          <th className='editServicesTable' scope='row' onClick={setDeploymentId}><Link data-id={item.id} to='/Services/services_1'>{item.name}</Link></th>
          <td></td>
          <td></td>
          <td>{item.created}</td>
